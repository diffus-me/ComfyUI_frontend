...
const onMonitorError = async ({ detail }: CustomEvent) => {
  useDialogService().showMonitorErrorDialog(detail.message.reason, detail.message.need_upgrade)
}
...
function showComfyUIBundleDialogIfNeeded() {
  ...
  useDialogService().showComfyUIBundleDialog()
}

function checkBundle() {
  const bundle = orderInfoStore.getBundle()
