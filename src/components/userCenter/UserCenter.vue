  ...
  <Toast position="bottom-right" group="notifier">
    <template #message="slotProps">
      <div class="flex flex-auto items-start">
        ...
        <span
          class="my-4 text-lg font-medium"
...
import {
  openUpgradeDialogByReason,
  showComfyUIBundleDialog
} from '@/scripts/diffusApp'
...
function checkComfyUIBundle() {
  ...
  if (bundlePromptStatus) {
    ...
    if (
      now.getMonth() === lastPromptedAt.getMonth() &&
      now.getFullYear() === lastPromptedAt.getFullYear()
    ) {
      return
    }
  }
...
onMounted(() => {
  ...
  console.log('Diffus UserCenter mounted')
})
...
onUnmounted(() => {
  ...
  console.log('Diffus UserCenter unmounted')
})
</script>
